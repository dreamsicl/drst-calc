...
app.factory('Data', function(){
  ...
  ret.getTimeLeft = function() { return ret.data.timeLeft };
  ret.setTimeLeft = function(time) { ret.data.timeLeft = time};
...
app.controller('timeCtrl', function($scope, $http, $timeout, Data) {
  ...
  $scope.$watch('deadline', function (newValue, oldValue) {
    $http.get(url).then(function(response) {
      $scope.deadline = response.data.gachas[0].end_date*1000;
    }, function(response) {
...
app.controller('liveController', function($scope, $filter, Data, NormalLive, TokenLive, Exp) {
  ...
  $scope.staminas = [];
  for(var i = 10; i < 20; i++){ $scope.staminas.push(i); };
  ...
  var lvlInfo = $filter('filter')(Exp, {"Level":$scope.user.lvl})[0];
  ...
  $scope.updateStatus = function() {
    ...
    lvlInfo = $filter('filter')(Exp, {"Level":$scope.user.lvl})[0];
    getExpInfo($scope.user.exp);
  ...
  var findNormByStam = $filter('filter')(NormalLive, {"Stamina": $scope.norm.stam})[0];
  ...
  $scope.updateNorm = function() {
    findNormByStam = $filter('filter')(NormalLive, {"Stamina": $scope.norm.stam})[0];
    searchNorm($scope.norm.score, $scope.norm.mul);
  };

  // event lives: get token cost, point worth, & exp
  var totalPtsTokn = $scope.tokn.cost + $scope.tokn.ptsEarned;
  var findToknByDiff = $filter('filter')(TokenLive, {"Difficulty": $scope.tokn.diff})[0];
  ...
  $scope.updateTokn = function() {
    findToknByDiff = $filter('filter')(TokenLive, {"Difficulty": $scope.tokn.diff})[0];
    searchTokn($scope.tokn.score, $scope.tokn.mul);
  };
  ...
  $scope.calcNormalLivesNeeded = function() {
    ...
    if (extraNorm > 0) { nPlay += extraNorm; }
    return nPlay;
  }
  ...
  $scope.timeLeft = 0;
  $scope.$watch(function() {return Data.getTimeLeft(); }, function(n,o) {
    if (n!==o) {
      $scope.timeLeft = n;
  ...
  $scope.calcStamDeficit = function() {
    var nPlay = $scope.calcNormalLivesNeeded();
    var stamDeficit = $scope.norm.stam * nPlay;
    ...
    stamDeficit -= staFromLevelUp - $scope.naturalStam;
    return Math.max(stamDeficit,0);
  }
